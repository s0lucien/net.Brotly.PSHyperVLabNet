function Setup-HyperVLabNet ($NetPrefix = "BrotlyNet") {
    ...
    ### Create vSwitches
    PSHyperVLabNet\New-VMSwitch -SwitchName "$($NetPrefix)_pf2vm" -SwitchType "Private"
    PSHyperVLabNet\New-VMSwitch -SwitchName "$($NetPrefix)_host" -SwitchType "Internal"
    ...
    PSHyperVLabNet\Connect-VMNetworkAdapter -VMName "pfSense" -SwitchName "$($NetPrefix)_pf2vm"
    ...
    Get-NetIPAddress -InterfaceIndex (Get-NetAdapter -Name "vEthernet ($($NetPrefix)_host)").ifIndex | Remove-NetIPAddress -confirm:$false
    New-NetIPAddress -InterfaceAlias "vEthernet ($($NetPrefix)_host)" -IPAddress "10.10.80.57" -PrefixLength 24
    ## for the VMs
    ...
    $hostName="pf.surf"
    $MacAddressWhereStaticIP = PSHyperVLabNet\Get-MACAddressFromString "$VMName-BrotlyNet_host"
